            ...
            <div className="p-8 md:p-10 flex flex-col md:flex-row items-start md:items-center gap-8">
              <div className="flex-1">
                <div className="inline-flex items-center gap-2 bg-blue-500/10 border border-blue-500/20 rounded-full px-3 py-1 mb-4">
                  <span className="w-1.5 h-1.5 rounded-full bg-blue-400 animate-pulse" />
                  <span className="text-blue-400 text-xs font-semibold">Full interactive CRM demo</span>
                </div>
                <h3 className="text-2xl font-bold mb-3">ATX Financial — Agency CRM</h3>
                <p className="text-gray-400 text-sm leading-relaxed mb-4 max-w-lg">
                  A complete insurance agency management platform with dashboard analytics,
                  client contacts, deal pipeline, task management, and performance reports.
                  Explore every tab with realistic mock data.
                </p>
                <div className="flex flex-wrap gap-2 mb-6">
                  {["Dashboard & KPIs", "Contact management", "Deal pipeline", "Task tracking", "Agent reports"].map((f) => (
                    <span key={f} className="text-xs bg-gray-800 border border-gray-700 text-gray-400 px-2.5 py-1 rounded-lg">{f}</span>
                  ))}
                </div>
                <div className="flex flex-wrap gap-3">
                  <Link
                    href="/demo/atx-financial"
                    className="inline-flex items-center gap-2 bg-blue-500 hover:bg-blue-400 text-white font-semibold text-sm px-6 py-3 rounded-full transition-colors"
                  >
                    Open CRM demo
              ...
              <div className="shrink-0 w-full md:w-64 bg-gray-950 border border-gray-800 rounded-2xl p-4 space-y-3">
                <p className="text-[10px] uppercase tracking-widest text-gray-600 mb-2">CRM Features</p>
                {[
                  { icon: "chart", label: "Dashboard", desc: "KPIs, charts, activity feed" },
                  { icon: "users", label: "Contacts", desc: "30 clients with full details" },
                  { icon: "funnel", label: "Pipeline", desc: "6-stage deal tracking" },
                  { icon: "tasks", label: "Tasks", desc: "Priority-based task board" },
                  { icon: "report", label: "Reports", desc: "Agent rankings & trends" },
                ].map(({ label, desc }) => (
                  <div key={label} className="flex items-start gap-3">
                    <div className="w-6 h-6 rounded-md bg-blue-900/40 border border-blue-800/40 flex items-center justify-center text-blue-400 text-[10px] font-bold shrink-0 mt-0.5">
                      {label[0]}
                    </div>
                    <div>
            ...
            <div className="bg-blue-950/40 border border-blue-800/40 rounded-2xl p-7">
              ...
              <ul className="space-y-4">
                {[
                  ...
                  "Validated input only. Dropdowns, not free-text fields.",
                  "Real-time sync. Everyone sees the same data, instantly.",
                  "Granular sharing — give a client read-only access to their records only.",
                ].map((fix) => (
