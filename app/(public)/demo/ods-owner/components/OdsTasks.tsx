...
const STATUS_LABELS: Record<OdsTask["status"], string> = {
  todo: "To Do",
  in_progress: "In Progress",
  done: "Done",
};

const STATUS_ORDER: OdsTask["status"][] = ["todo", "in_progress", "done"];
const PRIORITY_OPTIONS: OdsTask["priority"][] = ["low", "medium", "high", "urgent"];

const PRIORITY_LABELS: Record<OdsTask["priority"], string> = {
  low: "Low",
  medium: "Medium",
  high: "High",
  urgent: "Urgent",
...
function isOverdue(task: OdsTask): boolean {
...
export function OdsTasks({
  ...
  const { tasks, members, clients } = state;

  const [statusFilter, setStatusFilter] = useState<"all" | OdsTask["status"]>("all");
  const [priorityFilter, setPriorityFilter] = useState<"all" | OdsTask["priority"]>("all");
  const [showForm, setShowForm] = useState(false);
  const [editingTask, setEditingTask] = useState<OdsTask | null>(null);
  ...
  const groupedTasks = useMemo(() => {
    const groups: Record<OdsTask["status"], OdsTask[]> = { todo: [], in_progress: [], done: [] };
  ...
  const emptyForm = {
    title: "",
    description: "",
    priority: "medium" as OdsTask["priority"],
    dueDate: "",
    assigneeId: "",
    relatedClientId: "",
    relatedProjectId: "",
  ...
  const openEditForm = useCallback((task: OdsTask) => {
    setForm({
      title: task.title,
  ...
  const handleSave = useCallback(() => {
    ...
    const assignee = members.find((a) => a.id === form.assigneeId);
    ...
    if (editingTask) {
      dispatch({
        ...
        updates: {
          ...
          assigneeName,
          relatedClientId: form.relatedClientId || undefined,
    ...
    setShowForm(false);
    setEditingTask(null);
  }, [form, editingTask, members, dispatch]);
    ...
    <div className="space-y-4">
      ...
      <div className="flex flex-wrap items-center gap-3">
        ...
        <select
          ...
          className="rounded-lg bg-[#0d1526] border border-gray-700 px-3 py-1.5 text-xs text-gray-300 outline-none focus:border-cyan-500/50"
        >
            ...
            <div className="space-y-3">
              ...
              <div className="grid grid-cols-2 gap-3">
                <div>
                  ...
                  <select
                    value={form.priority}
                    onChange={(e) => setForm({ ...form, priority: e.target.value as OdsTask["priority"] })}
                    className="w-full rounded-lg bg-[#0d1526] border border-gray-700 px-3 py-2 text-xs text-gray-200 outline-none focus:border-cyan-500/50"
                  >
                    {PRIORITY_OPTIONS.map((p) => (
                      <option key={p} value={p}>{PRIORITY_LABELS[p]}</option>
              ...
              <div>
                <label className="block text-[10px] font-semibold text-gray-400 uppercase tracking-wider mb-1">Related Client</label>
                <select
                  value={form.relatedClientId}
                  onChange={(e) => setForm({ ...form, relatedClientId: e.target.value })}
                  className="w-full rounded-lg bg-[#0d1526] border border-gray-700 px-3 py-2 text-xs text-gray-200 outline-none focus:border-cyan-500/50"
                >
